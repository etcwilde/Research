...
FROM
(SELECT commits.cid, cast(strftime('%Y', commits.created_at) AS INTEGER) valYear, 100. * no_test.churn / (commits.lines_added + commits.lines_removed) 'src_percent', 100. * with_test.churn / (commits.lines_added + commits.lines_removed) test_percent
FROM commits
JOIN (SELECT cid, sum(added) + sum(removed) churn FROM files WHERE filename NOT LIKE '%test%' GROUP BY cid) AS no_test
ON commits.cid = no_test.cid
JOIN (SELECT cid, sum(added) + sum(removed) churn FROM files WHERE filename LIKE '%test%' GROUP BY cid) AS with_test
...
# Make the plot
svg("test_ratio_churn_distribution_over_time.svg", width=16, height=10)
...
files_plot <- annotate_figure(files_plot, top="Ratio of Test to Source Churn Per Commit Over Time")
plot(files_plot)
